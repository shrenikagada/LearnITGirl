...
def test_tuples_are_immutable():
    ...
    try:
        ...
        print("See me in pytest output?", te, sep="\n") # note the exception "'tuple' object does not support item assignment"
        assert True
...
def test_tuple_unpacking():
    ...
    try:
        ...
        print("See me in pytest output", ve, sep="\n") # observe what is printed here. "too many values to unpack (expected 2)"
        assert True
...
def min_max(input):
    ...
    """
    return (min(tuple(input)),max(tuple(input)))  # write a line of code to return containing min and max
