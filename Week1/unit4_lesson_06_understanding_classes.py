...
def test_classes_bound_and_unbound_methods():
    ...
    #print(TestQueue.push.__self__())   #unbound method
    #print(q1_push.__self__())     #bound method, is associated with an instance

    # now understand the output of these 2 statements.
    #assert "<bound method test_classes_bound_and_unbound_methods.<locals>.TestQueue.push of <unit4_lesson_06_understanding_classes.test_classes_bound_and_unbound_methods.<locals>.TestQueue object at 0x034D6770>>" == (q1.push)
    #assert  "<function test_classes_bound_and_unbound_methods.<locals>.TestQueue.push at 0x034CD858>" == (TestQueue.push)
...
class Person:
    ...
    def get_bmi_result(self):
        val = self.height*0.12
        val*=2.54
        bmi=(self.weight)/(val*val)
        print(bmi)
        if bmi < 18 :
            return "underweight"
        elif bmi > 50:
