...
def test_debug_sort_by_len():
    ...
    assert ['a', 'ab', 'bc', 'abc', 'abcd'] == (buggy_dedup_sort_by_len(["bc","ab","a","abcd","a", "abc"]))
    assert ["a"] == buggy_dedup_sort_by_len(["a","a"])
    assert None == buggy_dedup_sort_by_len(None)
    assert [] == buggy_dedup_sort_by_len([])
...
def sort_by_vowel_count(words):
    if(words!=None):
        data1 = sorted(words,key=vowel,reverse=True)
        return data1
    return None;

def single_sort_by_vc_test(input, result):
    data = sort_by_vowel_count(input)
    assert result == data
    #return data
