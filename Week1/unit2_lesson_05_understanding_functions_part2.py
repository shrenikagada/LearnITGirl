...
def test_function_call_with_keyword_arguments():
    ...
    assert [10,20,10,4] == demo(first=10,second=20, third=10)  # is this allowed? correct and uncomment
...
def simple_format(format, *args):
    ...
    class T(Template):
        delimiter = '%'
        idpattern = '\d+'
    return T(format).safe_substitute({str(i):v for i,v in enumerate(args)})
